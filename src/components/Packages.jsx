...
function ProgramCard({ pkg }) {
  ...
  return (
    <div
      className={`group reveal rounded-[2.5rem] p-8 lg:p-10 relative overflow-hidden border transition-all duration-500 hover:-translate-y-2 flex flex-col justify-between ${
        isFeatured
    ...
    >
      ...
      <div>
        {/* Icon */}
        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-2xl mb-6 border ${
          isFeatured 
        ...
        {/* Title */}
        <h3 className={`font-serif text-[1.4rem] font-light mb-3 leading-tight transition-colors duration-300 ${
          isFeatured 
        ...
        {/* Description */}
        <p className={`text-[13.5px] leading-relaxed font-light ${
          isFeatured ? 'text-cream/75' : 'text-taupe'
        }`}>
          {pkg.desc}
        </p>
        ...
        <button
          ...
          className={`text-[9.5px] font-bold tracking-[0.18em] uppercase flex items-center gap-1.5 cursor-pointer mt-4 mb-6 select-none transition-colors duration-300 outline-none border-none ${
            isFeatured 
        ...
        <div className={`transition-all duration-500 ease-in-out card-scrollbar ${
          isExpanded 
          ...
          {pkg.idealFor && pkg.idealFor.length > 0 && (
            <div className="mb-6">
              <h4 className={`text-[9.5px] font-bold tracking-wider uppercase mb-3 ${
                isFeatured ? 'text-cream/50' : 'text-brown/65'
              }`}>
                Ideal For:
              </h4>
              <div className="flex flex-wrap gap-1.5">
                {pkg.idealFor.map((item, id) => (
                  <span
                    key={id}
                    className={`text-[11px] py-0.5 px-2.5 rounded-full border font-light ${
                      isFeatured
          ...
          {pkg.mayInclude && pkg.mayInclude.length > 0 && (
            <div className="mb-6">
              <h4 className={`text-[9.5px] font-bold tracking-wider uppercase mb-2.5 ${
                isFeatured ? 'text-cream/50' : 'text-brown/65'
              }`}>
                Program May Include:
              </h4>
              <ul className="space-y-1.5">
                {pkg.mayInclude.map((item, id) => (
                  <li
                    key={id}
                    className={`flex items-center gap-2 text-[12.5px] font-light ${
                      isFeatured ? 'text-cream/80' : 'text-taupe'
                    }`}
                  >
                    <span className={`w-1 h-1 rounded-full flex-shrink-0 ${
                      isFeatured ? 'bg-rose' : 'bg-rose/60'
                    }`} />
                    {item}
                  </li>
                ))}
      ...
      <div className="mt-4">
        ...
        <div className={`pt-5 border-t ${
          isFeatured ? 'border-white/12' : 'border-brown/5'
        }`}>
          <span className="text-[9px] font-bold tracking-wider uppercase text-rose block mb-1">
            Goal:
          </span>
          <p className={`text-[13px] leading-relaxed font-serif italic ${
            isFeatured ? 'text-cream/85' : 'text-taupe'
          }`}>
            {pkg.goal}
          </p>
        </div>
        ...
        <a
          href="#book"
          className={`block w-full text-center mt-6 py-3 px-6 rounded-full text-[10px] font-bold tracking-[0.2em] uppercase transition-all duration-300 ${
            isFeatured
